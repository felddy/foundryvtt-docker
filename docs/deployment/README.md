...
| [Audio/video chat with LiveKit](livekit-av/README.md) | Runs a [LiveKit](https://livekit.io) server beside Foundry so every player uploads their camera stream once instead of once per player.  Includes Compose and Kubernetes examples. |

Kubernetes users generally front the server with a Gateway API `HTTPRoute`
instead; see the [Kubernetes guide](kubernetes/README.md) for an example.
