...
|`felddy/foundryvtt:9.240.0`| An exact image version. |
|`felddy/foundryvtt:9.240`| The most recent image matching the major and minor version numbers. |
...
distribution's name must be of the form: `foundryvtt-9.240.zip`
...
| `FOUNDRY_VERSION` | Version of Foundry Virtual Tabletop to install. | `9.240` |
...
```console
docker build \
  --build-arg VERSION=9.240.0 \
  --tag felddy/foundryvtt:9.240.0 \
