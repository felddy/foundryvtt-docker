...
|`felddy/foundryvtt:0.7.8`| An exact version. |
...
distribution's name must be of the form: `foundryvtt-0.7.8.zip`
...
|-------|---------|---------|
| `CHOWN_EXCLUDE_USER_MOUNT` | Specify a mount point to be excluded when during the `chown` step during startup. This is useful if you want mount a volume as read only inside `/data` (e.g. mounting a volume that contains assets in `/data/Data/asssets`). | |
