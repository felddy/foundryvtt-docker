...
distribution's name must be of the form: `foundryvtt-9.232.zip`
...
| `FOUNDRY_IP_DISCOVERY` | Allow the container to discover the host machine's public IP address.  This is useful if you are running a container in a private network and need to access the game session from a different network.  Setting this to `false` may reduce startup times when this discovery would timeout. | `true` |
...
| `FOUNDRY_VERSION` | Version of Foundry Virtual Tabletop to install. | `9.232` |
