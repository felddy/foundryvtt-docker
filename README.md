...
|`felddy/foundryvtt:0.7.5`| An exact version. |
...
| `CONTAINER_PRESERVE_CONFIG` | Normally new `options.json` and `admin.txt` files are generated by the container at startup.  Setting this to `true` preserves these files and prevents the container from managing them.  This means that changes to environment variables will not be applied to the server. | `false` |
